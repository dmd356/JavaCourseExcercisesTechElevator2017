...
//	 */
	public String animalGroupName(String animalName) {
		...
		if(animalName == null || animalName.isEmpty()){
			...
			if(myMap.containsKey(lowercasename)){
				return myMap.get(lowercasename);
			}else {
				return "unknown";
			}
		}
...
//	 */
	public Double isItOnSale(String itemNumber) {
		...
		String tolowercase = itemNumber.toLowerCase();
				if(itemNumber == null || itemNumber.isEmpty()){
					return 0.00;
				}else{
					
					if(myMap.containsKey(tolowercase)){
						return myMap.get(tolowercase);
					}else {
						return 0.00;
					}
				}
...
//	 */
	public Map<String, Integer> peterPaulPartnership(Map<String, Integer> peterPaul) {
		Integer peterMoney = peterPaul.get("Peter");
		Integer paulMoney = peterPaul.get("Paul");
		Map<String, Integer> ppp = new HashMap<String, Integer>();
		ppp.put("Peter", peterMoney.intValue());
		ppp.put("Paul", paulMoney.intValue());
		
		if(paulMoney >= 10000 && peterMoney >= 5000){
			Integer sum = paulMoney.intValue()/4 + peterMoney.intValue()/4;
			ppp.put("PeterPaulPartnership", sum);
			ppp.put("Peter", (peterMoney.intValue() - peterMoney.intValue()/4));
			ppp.put("Paul", (paulMoney.intValue() - paulMoney.intValue()/4));
			return ppp;
...
//	 */
	public Map<String, String> beginningAndEnding(String[] words) {
		Map<String, String> myMap = new HashMap<String, String>();
		for(String x : words){
			myMap.put(x.substring(0, 1), x.substring(x.length() -1));
		}
...
//	 
	...
	public Map<String, Integer> consolidateInventory(Map<String, Integer> mainWarehouse, Map<String, Integer> remoteWarehouse) {
		...
		for(String key : mainWarehouse.keySet()){
			myMap.put(key, mainWarehouse.get(key));
			
		}
		for(String key : remoteWarehouse.keySet()){
			if(myMap.containsKey(key)){
				Integer count = myMap.get(key);
				myMap.put(key, remoteWarehouse.get(key) + count);
			}else{
				myMap.put(key, remoteWarehouse.get(key));
			}
		}
		return myMap;
	}
	...
	public Map<String, Integer> last2Revisted(String[] words) {
		...
		for(String word : words){
			myMap.put(word, last2count(word));
		}
		return myMap;
		
	}
	public int last2count (String str){
