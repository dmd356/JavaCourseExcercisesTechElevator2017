...
from city where district = 'Ontario';

-- 2. The name and population of all cities in Montana (1 row)
select population, name
from city where district = 'Montana';

-- 3. The name, form of government, and head of state of all countries in Europe (46 rows)
select name, headofstate, region, governmentform
from country where continent like '%Europe';
...
from country where continent like '%Asia';
...
from country where gnp > 1000000;
...
from country where gnp > 1000000 and population < 100000000
...
select name, gnp, continent, lifeexpectancy from country where gnp between 1000000 and 100000000 and lifeexpectancy >= 70;

-- 21. The per capita GNP (i.e. GNP divided by population) in US Dollars of all countries in Europe (46 rows)

select gnp / population as newgnp from country where continent = 'Europe';
